    ...
    <div className='productItem group w-full rounded-md overflow-hidden bg-white shadow-xl border border-black flex md:flex-row flex-col gap-4'>
      ...
      <div className='info p-4 flex flex-col justify-between w-full bg-[#f1f1f1]'>
          ...
          <p>
            {
              props?.item?.description
                ...
                .slice(0, 60)              // take first 60
                .join(" ") +               // join back
              (props?.item?.description?.split(" ").length > 60 ? "..." : "")
            }
          </p>
        ...
        <div className='flex items-center gap-4 mt-3'>
          <span className='oldPrice line-through text-gray-500 text-[15px] font-[500]'>&#x20b9;{Number(props?.item?.oldPrice).toLocaleString("en-IN")}</span>
          <span className='price text-primary text-[15px] font-[600]'>&#x20b9;{Number(props?.item?.price).toLocaleString("en-IN")}</span>
        </div>

        <Button className='btn-Org  !text-white w-44 !gap-3'><IoCartOutline className='text-[19px]' />Add to Cart</Button>
      </div>
    </div>
  );
}
